...
// Populate settings panel with persisted values
angleModeSelect.value = config.angleMode || "deg";
modeSelect.value = config.calculatorMode || "standard";
themeSelect.value = config.theme || "light";


// Apply persisted angle mode
let angleMode = config.angleMode || "deg";

// Persisted calculator mode
let initialMode = config.calculatorMode || "standard";

document.addEventListener("DOMContentLoaded", () => {
  const targetBtn = document.querySelector(
    `.breadcrumb-btn[data-mode="${initialMode}"]`
  );
...
// Breadcrumb navigation with persistence
breadcrumbButtons.forEach(btn => {
  btn.addEventListener("click", async () => {
    ...
    // Persist choice
    await updateConfig({ calculatorMode: mode });

    // Log event
    await logEvent("calculator_mode_changed", { mode });
  });
});
...
async function handleFunction(func) {
  try {
    switch (func) {
      ...
      case "deg":
        angleMode = "deg";
        await updateConfig({ angleMode: "deg" });
        await logEvent("angle_mode_changed", { angleMode: "deg" });
        break;

      case "rad":
        angleMode = "rad";
        await updateConfig({ angleMode: "rad" });
        await logEvent("angle_mode_changed", { angleMode: "rad" });
        break;
